...
export default function ProductsPage() {
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      setProducts((prev) =>
        prev.map((p) => (p.id === selectedProduct.id ? { ...p, ...editForm, media_urls: uploadedUrls.filter((url): url is string => url !== undefined) } : p))
      );
      setEditOpen(false);
            ...
            {products.map((product) => (
              <Card key={product.id} className="flex flex-col p-2">
                <CardContent className="flex-1 p-0">
                  ...
                  <CardTitle className="truncate pb-1 mt-2">{product.name}</CardTitle>
                </CardContent>
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditForm({
                      ...editForm,
                      unit_quantity: e.target.value ? Number(e.target.value) : null,
                    })
                  }
                />
              </div>
              <div className="grid grid-cols-4 items-start gap-4">
                <Label className="col-span- lager-1 font-medium">Media:</Label>
